...
def insertMedia(name, absolute, src):
	collection.update({"absolute":absolute}, { "$set": { "name":name, "absolute":absolute,
						"src":src } }, upsert=True)

def getAllMedia():
	#return list(collection.find({},{"src": 1, "_id":False},exhaust=True))
	return dumps(list(collection.find({},{"_id":False,"absolute":False},exhaust=True)))